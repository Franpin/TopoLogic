...
class TopoNetHead(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, bev_feats, img_metas, te_feats, te_cls_scores):
        ...
        hs, init_reference, inter_references, lclc_rel_out, lcte_rel_out, outputs_coords = outputs
    ...
    def get_lanes(self, preds_dicts, img_metas, rescale=False):
        ...
        lanes_preds = lanes_preds.reshape(lanes_preds.shape[0],lanes_preds.shape[1],-1,3)
        ...
        # topo = torch.exp(-(topo*topo)/(11.5275))*topo_mask
            # 

        
        a = topo.detach().cpu().numpy()
        # b = preds_dicts['all_lclc_preds'][-1].squeeze(-1).sigmoid().detach().cpu().numpy()
        b = preds_dicts['all_lclc_preds'][-1].squeeze(-1).detach().cpu().numpy()
        # all_lclc_preds = lamda_1*b + lamda_2*a
        all_lclc_preds = b

        # from IPython import embed; embed()
        # all_lclc_preds = topo.detach().cpu().numpy()
